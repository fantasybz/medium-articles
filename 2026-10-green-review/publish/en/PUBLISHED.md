...
以 `./tools/medium_draft.sh 2026-10-green-review en --post 4d36d0f2f9c1` 就地更新。等待儲存完成後，另開新分頁
...
封面維持 `diagram-02.png`（`1*7E7igGi0v150j59JclJy9g.png`）。重新讀取發布設定，
確認 Post ID、五個 Topics、訂閱通知、預覽標題與摘要、原排程均與更新前相同。
文章仍排定於 **2026-10-22（四）09:00 GMT+8** 發布；本次沒有重新排程或提前發布。
...
封面維持 `diagram-02.png`（`1*7E7igGi0v150j59JclJy9g.png`）。文章仍排定於 **2026-10-22（四）09:00 GMT+8** 發布；本次沒有重新排程或提前發布。
...
更新後從新分頁讀回，200 個文字區塊、36 個連結、7 張圖片、12 條分隔線皆與貼稿相符；連結順序與目的地、圖片槽位與識別碼也逐項核對。再次讀取發布設定，確認本篇原排程、Post ID、封面、Topics、預覽文字與通知設定均未改動。
