...
作者要求以「購買無糖純喫綠茶」完整解釋三種選擇：從 Review 留下哪些 constraint tests、依風險選誰審閱哪些內容、選哪些程式與 mutant 來檢查測試保護力。依作者要求，系列採用「總論＋四部曲」，共五篇文章。既有總論與前三篇各補對應段落，第四篇〈付款實作篇〉保留連續的契約、程式、實驗與判讀。中英文共十份原稿同步更新。第四篇初次交付時未指定日期；後續依作者要求於同日完成 Medium 排程：中文 2026-11-03、英文 2026-11-05，皆為 09:00 GMT+8。正文以篇名指引讀者，實際時程記於 [發布佇列](../../PUBLISHING.md)。
...
- 兩種語言保留相同條件、數字與未完成範圍。既有八篇排程不改；第四篇中英文草稿後續依作者要求接續排程，並從 Medium 重新讀取時間戳核對。
