...
Today is ${args.today}. You are writing for @fantasybz (Kochi Chuang): long-form Traditional-Chinese (Taiwan usage; English technical terms untranslated) Medium articles for Engineering VPs / EMs / Staff engineers / platform, SRE and QA leads in Taiwan. Shared voice and clarity rules are in ${ROOT}/STYLE.md; series structure and tail conventions are in ${DIR}/style_brief.md — follow them (TL;DR blockquote, 系列導覽 line, ## 一、二、… sections, 結語 returning to the opening question or documented experience (a blockquote is optional), ### 系列文章 → ### References → ### AI 協作說明 → italic signature line; single "—" only, never "——").
...
const LENSES = [
  ...
  { key: 'reader', prompt: `LENS: The reader. (Read only the article, the plan's 主題總覽 and style_brief.md.) You are an Engineering VP at a 300-person Taiwanese software company who has read the author's published pieces. Would you finish it and forward it? Where does it drag, repeat, or lecture? Is the thesis argued or asserted? Which decision artefacts are real? Flag padding, unsupported hedging, unclear actions and gaps in reasoning; keep narrative and explanatory paragraphs that help the reader understand the decisions.` },
  { key: 'editor', prompt: `LENS: Senior tech editor. (Read the article, style_brief.md, the plan, and the published articles.) Check the format skeleton exactly (TL;DR / 系列導覽 / numbered sections / 結語 / 系列文章 / References / AI 協作說明 / signature), single "—", Traditional Chinese Taiwan usage (flag mainland terms and translationese), reading time vs target (count CJK characters outside code blocks with python; 總論 target 3,500–4,500, the published 總論 has 3,629; treat the character range as a scope guide, respect author-approved length in style_brief.md, and flag redundancy without cutting explanations or story transitions needed for clarity), overlap with the published pieces and with sibling pieces of this theme, figure placement and captions, tables' width (≤ 5 columns for Medium), and that every link in the text is either a real URL or the plain-text 「（即將發布）」 marker.` },
]
phase('Critique')
