...
const LENSES = [
  ...
  { key: 'editor', effort: 'high', prompt: `LENS: Senior tech editor. (Read only the outline, style_brief.md, selection.md and the published articles' headings — do not open the digests.) Check (a) format against style_brief.md: TL;DR, 系列導覽, Chinese-numeral section numbering, 結語 returning to the opening question or documented experience, 系列文章 / References / AI 協作說明 tail, single "—" dash, Traditional Chinese Taiwan usage (flag mainland terms and translationese); (b) overlap with the published articles (repeating instead of extending is a major) and between the 總論 and the three parts and among the parts; (c) whether the four pieces plus an English edition can realistically be written in one month by one author with an AI collaborator — flag over-scoped sections and say what to cut; (d) the figure/table plan: is each figure specified well enough to draw, and are there 10–14 for the 總論 and 4–6 per part; (e) the References list: only sources present in the digests, formatted as the author does.` },
]
