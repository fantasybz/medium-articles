# 綠燈不是驗收・付款實作篇：買一瓶無糖純喫綠茶，從 Review 約束走到 mutation score

> **TL;DR** — 想買一瓶無糖純喫綠茶，付款畫面卻遲遲沒有回應。再按一次會不會多扣一筆？這篇用一筆示範售價 35 元的訂單，把「綠燈不是驗收」的三道閘串起來：先依風險挑選 reviewer 與需要深讀的變更，再從 Review 意見中挑出值得長期保護的約束，把它們寫成測試，最後選擇相關程式碼做 mutation testing。隨文附上可執行的 Python 範例。實際執行七個指定 mutant，只測正常購買時殺死 2 個，mutation score 是 28.6%；補到 8 個測試、只缺逾時案例時，分數已達 85.7%，卻仍抓不到「把不確定的付款標成成功」；完整 9 個測試才辨識出這七種變異。100% 只描述這次選定的範圍，也不能替並行、重新啟動與真實金流整合作保。

> 系列導覽：[總論：綠燈不是驗收](https://medium.com/p/582f24223eea) → [測試篇](https://medium.com/p/b01055139451) → [Review 篇](https://medium.com/p/ccbf0cbe2691) → [可靠度篇](https://medium.com/p/3c64a9622777) → **付款實作篇（本篇）**。本篇可以獨立閱讀，四篇主文則提供方法的研究背景與組織安排。
...
先確認未修改的程式通過所選測試，再每次只改一處，執行同一組測試，最後丟棄那份變異程式。測試因預期行為差異失敗，代表這個 mutant 被辨識，通常稱為 killed；沒有失敗則是 survived。
...
我用隨文的 `mutation_demo.py` 實際執行了三組測試選擇，原始程式在每一組都先通過。結果如下：

| 測試選擇 | killed／納入計分 | mutation score | 仍存活的變異 |
|---|---|---|---|
| 只有 2 個正常購買測試 | 2／7 | 28.6% | M2、M3、M4、M5、M7 |
| 8 個測試，漏掉逾時案例 | 6／7 | 85.7% | M5 |
| 完整 9 個測試 | 7／7 | 100.0% | 這七個都被辨識 |

第一列的兩個測試都是真的，也都會執行付款；它們只是沒有要求程式處理重送、key 衝突、拒絕與逾時。mutation 讓這個保護範圍變得可見。

第二列更值得停一下。如果團隊只採用測試篇提出的 70% 起始建議，85.7% 已經超過門檻。但 C4 的逾時情境仍然沒有被保護：M5 把 `PENDING` 改成 `PAID`，八個測試完全沒有抗議。**總分過線，不能抵銷尚未驗證的關鍵付款約束。** 這筆 PR 應先補足逾時案例，並確認它真的會對 M5 失敗。
...
Mutation 報告也要保留失敗原因。本例 runner 先確認原始程式通過，也核對測試選擇與數量沒有漂移；再把每個 mutant 放進獨立的暫存目錄執行，列出失敗的測試名稱。若發生 import／執行錯誤、沒有收集到測試，或執行逾時，就中止示範，不把這些情況悄悄算成 killed。這是本教學採用的保守計分方式，不是所有正式工具的預設分類。
...
目錄裡另有十二個 runner 回歸測試，用來確認載入失敗、測試異常或選擇漂移不會變成可信的報告。它們不計入九個付款測試，也不計入七個 mutant 的分母；若用 `test_*.py` 執行整個目錄，合計會看到 21 個測試。
...
那瓶綠茶的價格在這裡只是示範。值得被認真對待的，是有人把付款交給了這套系統，而團隊需要能解釋：我們確認過什麼，還有哪些問題需要繼續驗證。
...
- [測試篇](https://medium.com/p/b01055139451)：如何確認測試真的辨識得出錯誤。
- [Review 篇](https://medium.com/p/ccbf0cbe2691)：人的判斷、風險分流與核准責任。
- [可靠度篇](https://medium.com/p/3c64a9622777)：把 reviewer 約束留下來，量測候選成果與重複執行。
- **付款實作篇（本篇）**：用同一瓶無糖純喫綠茶，把選擇、測試與證據連起來。
