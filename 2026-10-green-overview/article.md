...
第三個數字是人力，這裡借一個 **code 以外的旁證**—概念用、數字不移植。READY 是企業 agent 部署的資格審查框架（2026 年 9 月），案例是臨床稽核工作流、16 個 agent 系統、750 個 case：兩個系統的自主準確率 72.8% 與 72.5%，只差 0.3 個百分點；要達到同一個 76% 的可靠度目標，所需人工複核比例卻分別是 39.2% 與 29.6%—準確率高的那一個要更多人看。**「準確率排名」與「你要付多少人力」不是同一個排名。** 「人工複核比例由可靠度目標反推」，本系列叫它 oversight budget，演算法在可靠度篇（下圖為 READY 的兩個系統，非 code 旁證）：
