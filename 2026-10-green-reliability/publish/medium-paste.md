...
先用一組數字定錨這一篇要處理的落差。SWE-Gate 是一份 benchmark 論文，量的就是綠燈到底涵蓋了什麼。它多做了一件事：不只問「功能測試過了沒」，還把 reviewer 當初真的在 PR 上要求過的東西抓出來，寫成可執行的檢查，跟功能測試分開跑一次。

34% 是 SWE-Gate 在 75 個 Python repo、303 個修補任務上量到的。644 個通過功能測試的修補裡，221 個違反了那些約束。用白話講就是：每三個綠燈修補，就有一個違反了 reviewer 在乎的約束。
