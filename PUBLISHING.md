...
### 目前的發布佇列（2026-09-07 決定：10 月系列排程發布）

九月系列八篇已全部上線（見各篇 `publish/PUBLISHED.md`）。10 月系列「綠燈不是驗收」改用**排程**：
八份草稿 2026-09-07 全部建好並逐塊比對通過，中文每週二 09:00、同一篇的英文版同週四 09:00（GMT+8；2026-09-23 重新讀取 Medium 排程時間核對）。
2026-09-07 實測：八篇在同一個晚上全部排完，沒有被「24 小時內最多發布或排程 2 篇」擋下—與 2026-09-02 把日期改成隔天被擋的經驗不同，差別可能在排程日期是否落在 24 小時內；先當作「排到一個月後不吃配額」，被擋再改回一天兩篇。

| 順位 | 文章 | Post ID | 排程（GMT+8） | 狀態 |
|---:|---|---|---|---|
| 1 | 總論（中） | `582f24223eea` | 10/06（二）09:00 | 已排程 2026-09-07 |
| 2 | 總論（英） | `c4fc9f3d8581` | 10/08（四）09:00 | 已排程 2026-09-07 |
| 3 | 測試篇（中） | `b01055139451` | 10/13（二）09:00 | 已排程 2026-09-07 |
| 4 | 測試篇（英） | `51d001a6dcd5` | 10/15（四）09:00 | 已排程 2026-09-07 |
| 5 | Review 篇（中） | `ccbf0cbe2691` | 10/20（二）09:00 | 已排程 2026-09-07 |
| 6 | Review 篇（英） | `4d36d0f2f9c1` | 10/22（四）09:00 | 已排程 2026-09-07 |
| 7 | 可靠度篇（中） | `3c64a9622777` | 10/27（二）09:00 | 已排程 2026-09-07 |
| 8 | 可靠度篇（英） | `4b6d147bff0d` | 10/29（四）09:00 | 已排程 2026-09-07 |

**付款實作補篇尚未排程。** 中英文原稿、貼稿與各 5 張圖表已在 repo 完成，Medium 同步待完成；資料夾名稱 `2026-10-green-payment` 不代表已決定發布月份。這次補充會更新上表八篇的內容，保留既有 Post ID、排程與發布設定；補篇只建立中英文草稿。

| 補篇 | Repo 原稿 | Medium 狀態 |
|---|---|---|
...
補篇的 [程式與重跑方式](examples/tea_payment/README.md)、[研究與實測紀錄](research/2026-10/payment-example-evidence.md) 已備妥。完成 Medium 同步後，再把實際 Post ID、草稿網址與逐塊比對結果寫入各語言包的 `PUBLISHED.md`，並更新本表；不要先填未確認的日期或網址。

草稿網址是 `https://medium.com/p/<id>/edit`；上線後 `https://medium.com/p/<id>` 會轉到正式網址，
所以系列連結用這個短網址就不必等上線才填。**連結只填「本篇發布時已經上線的篇」**，不留死連結：
中文總論不連三部曲；測試篇連總論；Review 篇連總論與測試篇；可靠度篇連前三篇。英文版另外連前面各篇的英文版與自己的中文版。
