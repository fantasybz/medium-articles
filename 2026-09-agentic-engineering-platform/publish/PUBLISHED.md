...
線上版逐塊比對過：128 個文字區塊全數相符、20 個連結全在、14 張圖各在原位。
...
所以技術篇與營運篇還沒發布（排定 9/3、9/4，草稿都已備妥）。線上版目前把這兩篇
寫成純文字並標「（即將發布）」，等它們上線後要換成真正的連結：
...
最新內容一律以 `article.md` 為準——它保留的是三篇都發布後的最終樣子（四個連結都是完整
Medium URL），線上版的「（即將發布）」只是過渡狀態。
