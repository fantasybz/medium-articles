...
`313bbb4` 用同一份 mermaid 設定把**四篇文章的所有 diagram 重新算繪過**，只有總論的
第七、八張是真的重畫；其餘只是換行與間距微調。線上版仍是重算前的版本。

本次只換了第七、八兩張（依指示不順手動其他圖）。用長寬比對照 14 張圖，13 張仍相符，
唯一量得出差異的是 `diagram-06.png`：repo 1250×2932（0.426），線上 700×1901（0.368），
代表它的換行確實變了。其餘因為版面沒動，長寬比一樣、看不出來，但位元組並不相同。
